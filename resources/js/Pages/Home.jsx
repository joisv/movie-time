...
export default function Home({
  ...
  return (
    ...
    <AuthLayout user={auth?.user} isDetail={isDetail} setIsDetail={setIsDetail} >
      ...
      <SwiperAuto>
        {recently_added.slice(0, 5).map((data, index) => (
          <SwiperSlide key={index} virtualIndex={index} >
            <div
              ...
              style={{
                backgroundImage: `url(storage/${data.backdrop_path})`
              }}
            >
              <div className='text-text mt-20 sm:space-y-2 space-y-1'>
                <h1 className=' font-bold sm:text-2xl text-xl'>{data.title}</h1>
                <div className='sm:flex sm:space-x-2 items-center sm:text-sm text-xs'>
                  ...
                  <span className='mr-1 sm:mr-0'>Duration: {data.release_date}</span>
                  <span>Imdb: {parseFloat(data.vote_average).toFixed(1)}</span>
                  <div className='flex space-x-2'>
                    <h1>Genre</h1>
                    {
                      data.genres.map((genre, index) => (
                        <p className='text-secondaryAccent' key={index}>{genre.map}</p>
                      ))
                    }
                  </div>
                </div>
                <p className='w-[80%] text-sm font-light text-secondaryAccent'>{shortSentence(data.overview)}</p>
