...
export default function Edit({ open, setOpen, datastreams, post }) {

    console.log(datastreams);
    
    const { data, setData, put, errors, processing } = useForm({
        streams: datastreams.streams,
        downloads: datastreams.downloads,
        post_id: datastreams.id
    })

    // useEffect(() => {
    //     setData({
    //         streams: datastreams.streams,
    //         downloads: datastreams.streams.flatMap(stream => stream.downloads),
    //         post_id: datastreams.id
    //     });
    // }, []);
    const addStream = () => {
                    ...
                    <div className='w-full p-2 rounded-md  items-center max-h-[80vh] '>
                        <form onSubmit={submit} className='md:space-y-5 sm:space-y-2 space-y-1'>
                            ...
                            <AsyncSelect
                                ...
                                placeholder={datastreams.title}
                                defaultValue={datastreams.title}
                            />
